...
public class FXTreeView extends Application
{
	...
	public void start(Stage primaryStage) throws RemoteException
	{
		...
		this.textFieldTop = new TextField();
...
//        ArrayList<TreeItem> products = helper.getProducts(planRoot);
	...
	private void getKids(Node parentNode, TreeItem parentTreeItem)
	{
		...
		for (int i = 0; i < parentNode.getChildren().size(); i++)
		{
			...
			Node nodeChild = parentNode.getChildren().get(i);
			
			EventType MouseEvent = null;
			nodeChild.addEventHandler(MouseEvent.MOUSE_PRESSED,
					(event) -> editTextField(nodeChild));
			
			nodeChild.addEventHandler(MouseEvent.MOUSE_ENTERED, 
...
// System.out.println(newValue.toString()));
	...
	private void editTextField(Node newValue)
	{
		this.textFieldTop.setText(newValue.toString());
		this.textFieldCenter.setText( ((Node) newValue).getData());
	}
